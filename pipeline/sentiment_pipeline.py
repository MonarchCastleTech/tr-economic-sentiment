...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["GDELT TR", "Reddit API", "Telegram", "Turkstat", "CBRT"],
            "version": "1.0.0"
        },
        "stats": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "GDELT TR"}
            for i in range(1, 16)
        ],
        "timeseries": [
