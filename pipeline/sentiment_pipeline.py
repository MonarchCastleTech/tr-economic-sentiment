# -*- coding: utf-8 -*-
"""Turkiye Economic Sentiment Radar - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["Google News RSS TR", "Reddit API", "Telegram", "Turkstat", "CBRT"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Sentiment Score", "value": "42/100", "delta": "+5%"},
            {"label": "Sources Tracked", "value": "156", "delta": "+2"},
            {"label": "Daily Signals", "value": "2,341", "delta": "-3%"},
            {"label": "Accuracy", "value": "78%", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "Google News RSS TR"}
            for i in range(1, 16)
        ],
        "timeseries": [
